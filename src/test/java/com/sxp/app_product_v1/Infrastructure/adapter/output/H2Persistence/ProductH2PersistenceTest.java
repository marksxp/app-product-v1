...
class ProductH2PersistenceTest {
    ...
    void givenFieldsAreInformed_WhenCalled_ShouldReturnProductDomain() {
        ...
        when(productRepository.findProductByFields(any(), any(), any())).thenReturn(Mono.just(product));
        ...
        StepVerifier.create(result)
                .assertNext(r -> assertEquals(productDomain.getStartDate(), r.getStartDate()))
                .verifyComplete();
    }

    @Test
    void givenProductRepository_WhenCalled_ShouldReturnException() {
        when(productRepository.findProductByFields(any(), any(), any())).thenReturn(Mono.error(new Throwable("error")));
